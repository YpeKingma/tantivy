...
pub fn intersect_scorers(mut scorers: Vec<RcRefCellScorer<Box<dyn Scorer>>>) -> RcRefCellScorer<Box<dyn Scorer>> {
    if scorers.is_empty() {
...
pub struct Intersection<TDocSet: DocSet, TOtherDocSet: DocSet = RcRefCellScorer<Box<dyn Scorer>>> {
    left: TDocSet,
    right: TDocSet,
